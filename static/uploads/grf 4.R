...
png(file="grf4.png", width=2000, height=2000, res=300)
ggplot(df, aes(x, fill=x)) +
  ...
  geom_line(aes(y = -1.2816*(1+1*(X.test[,1]>-0.8))), linetype = "dashed") +
  geom_line(aes(y = 1.2816*(1+1*(X.test[,1]>-0.8))), linetype = "dashed") +
  theme(legend.position = c(0.85, 0.75), 
        legend.direction = 'vertical', 
        plot.title = element_text(hjust=0.5),
        legend.background = element_rect(fill="lightgray", size=0.5)) +
  ggtitle("(edge) scale shift comparision") + xlab("X") + ylab("Y") +
