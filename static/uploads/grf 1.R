...
ggplot(df, aes(x, fill=x)) +
  ...
  geom_line(aes(y = 0.8*1*(X.test[,1]>0), colour = "truth")) +
  geom_line(aes(y = -1.2816 + 0.8*1*(X.test[,1]>0)), linetype = "dashed") +
  geom_line(aes(y = 1.2816 + 0.8*1*(X.test[,1]>0)), linetype = "dashed") +
  theme(legend.position = c(0.15, 0.87), 
        legend.direction = 'vertical', 
        plot.title = element_text(hjust=0.5),
        legend.background = element_rect(fill="lightgray", size=0.5)) +
  ggtitle("mean shift comparision") + xlab("X") + ylab("Y") +
